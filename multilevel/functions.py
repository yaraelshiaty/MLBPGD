...
def kl_distance(x: torch.tensor, proj: mgproj.TomoTorch, b: torch.tensor):
    """
    Computes the Kullback-Leibler (KL) divergence between the forward projection of x and b.
...
def BSMART_general(f, x: torch.tensor, tau, l, u, logv=None):
    """
    Generalized Bregman SMART update for box constraints.
    Performs a mirror descent step with box constraints using the Bregman divergence.
    Returns the updated variable and logv.
    """
...
def BSMART_general_ml(f, x: torch.Tensor, tau, l, u, logv=None, **context):
    """
    Multi-level compatible Bregman SMART update for box constraints.
    Accepts arbitrary **context for use with MultiLevelOptimizer.
    Returns the updated variable and logv.
    """
...
def mirror_descent_IS(f, x:torch.tensor, tau, l, **context):
    """
    Mirror descent update on the orthant using Burg entropy as mirror map.
    Returns a scalar torch tensor with gradient tracking enabled.
    """
    fx = f(x)
    fx.backward(retain_graph=True)
...
def bpgd_newton_solver(f, x, tau, lh, uh=None, *args, **context):
    ...
    Solves for the optimal step using a root-finding method.
    Returns the updated variable and the new theta.
    """
    fx = f(x)
    fx.backward(retain_graph=True)
    xgrad = x.grad
...
def Doptdesign(x: torch.Tensor, H: torch.Tensor):
    """
    Computes the negative log-determinant of the Fisher information matrix for D-optimal design.
    Returns a scalar torch tensor with gradient tracking enabled.
    """
    X = torch.diag(x)
    FIM = H @ X @ H.T
